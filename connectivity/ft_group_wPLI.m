function group_outfile = ft_group_wpli(subjList, cond, freqLabel, varargin)
% ft_group_wpli
% Group-level aggregation of subject-wise wPLI matrices (single-band files).
%
% INPUTS
...
%      <OutDir>/group_wpli_<cond>_<freqLabel>.mat
%
% SAVED CONTENT (MAT)
...
% - Assumes per-subject files contain wpli_mat and ROIs (with .name)
% -------------------------------------------------------------------------
...
for i = 1:nSubj
    subj = subjList{i};
    infile = fullfile(opt.BaseDir, subj, sprintf('wpli_%s_%s.mat', cond, freqLabel));

    if exist(infile,'file')~=2
        subj_missing{end+1} = subj; %#ok<AGROW>
        if opt.Verbose
            warning('Missing wPLI file: %s', infile);
        end
        continue;
    end

    try
        S = load(infile, 'wpli_mat', 'ROIs');
    catch ME
    ...
    if ~isfield(S,'wpli_mat') || isempty(S.wpli_mat) || ndims(S.wpli_mat) < 2
        subj_invalid{end+1} = subj; %#ok<AGROW>
        if opt.Verbose
            warning('Invalid wpli_mat in %s', infile);
        end
        continue;
    end
    ...
    if isempty(all_mats)
        ...
        if size(S.wpli_mat,1) ~= nROI || size(S.wpli_mat,2) ~= nROI
            subj_invalid{end+1} = subj; %#ok<AGROW>
            if opt.Verbose
                warning('Size mismatch in %s: wpli_mat is %dx%d but ROIs has %d.', ...
                    infile, size(S.wpli_mat,1), size(S.wpli_mat,2), nROI);
            end
            all_mats = []; roi_labels = {};
            continue;
        end
        ...
        if numel(S.ROIs) ~= numel(roi_labels) || size(S.wpli_mat,1) ~= numel(roi_labels)
            subj_invalid{end+1} = subj; %#ok<AGROW>
            if opt.Verbose
        ...
        % Enforce consistent label order (strict)
    ...
    M = double(S.wpli_mat(:,:,1));

    % wPLI is undirected in your use-case; enforce symmetry for clean group plots
    % (if the input is already symmetric this is a no-op)
    M = (M + M.') ./ 2;

    % Optional: ignore diagonal (self-links)
...
if opt.Verbose
    fprintf('[ft_group_wpli] Valid subjects: %d | missing: %d | invalid: %d\n', ...
...
if opt.MakeFigure
    ...
    title(sprintf('Group mean wPLI (%s, %s), n=%d', cond, freqLabel, nValid), 'Interpreter','none');
    colorbar;
end

% -------------------- Save outputs --------------------
group_outfile = fullfile(opt.OutDir, sprintf('group_wpli_%s_%s.mat', cond, freqLabel));
save(group_outfile, ...
...
if opt.Verbose
    fprintf('[ft_group_wpli] Saved: %s\n', group_outfile);
end

% -------------------- CSV export --------------------
if opt.ExportCSV
    csvfile = fullfile(opt.OutDir, sprintf('group_wpli_%s_%s_mean.csv', cond, freqLabel));
    writematrix(group_mean, csvfile);
    if opt.Verbose
        fprintf('[ft_group_wpli] CSV exported: %s\n', csvfile);
    end
end

% -------------------- QC text summary --------------------
qcfile = fullfile(opt.OutDir, sprintf('group_wpli_%s_%s_summary.txt', cond, freqLabel));
fid = fopen(qcfile, 'w');
assert(fid>0, 'Could not open QC file for writing: %s', qcfile);

fprintf(fid, 'Group wPLI summary\n');
fprintf(fid, 'Condition: %s\nBand: %s\nValid subjects: %d\n\n', cond, freqLabel, nValid);

fprintf(fid, 'ROI-wise strongest connections (by magnitude)\n');
...
if opt.Verbose
    fprintf('[ft_group_wpli] QC summary saved: %s\n', qcfile);
end
